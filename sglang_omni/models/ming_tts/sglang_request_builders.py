    ...
    *,
    ...
    def result_adapter(data: MingTTSSGLangRequestData) -> StagePayload:
        ...
        if data.stop_step is not None:
            if data.generated_last_chunk and not bool(data.generated_last_chunk[-1]):
                raise RuntimeError(
            ...
            if normalized is not None and not (
                ...
                raise RuntimeError(
                    ...
                    f"{data.stop_step}, but SGLang finished_reason was {raw!r}"
                )
            finish_reason = "stop"
        ...
        elif len(data.generated_latents) >= int(data.max_decode_steps):
            finish_reason = "length"
        else:
            finish_reason = "stop"
        ...
        state.generated_last_chunk = [bool(item) for item in data.generated_last_chunk]
