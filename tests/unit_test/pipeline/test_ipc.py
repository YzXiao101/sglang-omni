...
@pytest.mark.asyncio
async def test_launcher_passes_one_resolved_tts_request_policy(
    tmp_path, monkeypatch
) -> None:
    config = _make_config(tmp_path)
    policy = TTSRequestPolicy(allowed_voices=("speaker",))
    resolve = Mock(return_value=policy)
    monkeypatch.setattr(PipelineConfig, "resolve_tts_request_policy", resolve)
    ...
    assert kwargs["tts_request_policy"] is policy
    assert "requires_uploaded_voice_for_named_voice" not in kwargs
    assert "supports_uploaded_voice_references" not in kwargs
